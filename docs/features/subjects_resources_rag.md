...
科目是学习数据的组织边界。资料库负责把教材、讲义、课件、笔记导入和真题材料解析为可检索知识块。RAG 来源负责在资料问答时把回答对应到具体文件和片段，让用户知道答案来自哪里。
...
5. 系统按资源来源和用途展示分类：教材资料、生成讲义、笔记导入、真题练习、其他资料。
6. 用户从知识点生成讲义后，讲义可以保存到笔记本，也可以同时导入资料库，成为后续问答、解题和讲义生成的检索资源。
7. 用户在对话或解题中使用资料问答时，后端通过 SSE 返回 `[SOURCES]` 帧。
8. 前端把来源挂到 assistant 消息上，UI 展示参考来源。
...
- 资料列表按资源类型分组：教材资料、生成讲义、笔记导入、真题练习、其他资料。
- 全量重建索引请求。
- RAG 流式问答来源解析。
- 笔记导入资料库后可成为 RAG 文档。
- 讲义保存为笔记时可选择同时导入资料库。
...
| 讲义回写 | `lib/components/library/lecture/lecture_page.dart` | 讲义保存为笔记，可同步导入资料库 |
...
- 资源分类当前由前端根据文件名和来源语义归类，后端如提供显式 `resource_type` 后应优先使用后端字段。
