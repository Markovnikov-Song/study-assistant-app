...
用户不直接编辑复杂积木图。用户看到的是四个入口，后端再把文档、运行配置和积木图谱组织起来。
...
- 当前图谱是后端安全校验用，不暴露给普通用户直接拖拽编辑。

## 下一步优先级

1. 扩展运行器类型：把选择题、错题训练、资料问答从配置真正渲染成可交互页面。
2. 做版本管理：改造前保存旧版本，支持回滚和对比。
3. 做资源库联动：小工具生成的讲义、卡片、错题结果写回资料库分类。
4. 做分享实体：从“复制说明”升级为导出包、局域网分享或服务器分享链接。
5. 做业务 E2E：覆盖生成、改造、运行、事件写入、资料生成卡片、复制分享。
