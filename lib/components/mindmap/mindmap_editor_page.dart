...
// ─────────────────────────────────────────────────────────────────────────────
// MindmapEditorPage — 单份导图的编辑器页面
// Tab 0: 手动编辑  Tab 1: AI 生成
// ─────────────────────────────────────────────────────────────────────────────

class MindmapEditorPage extends ConsumerStatefulWidget {
  final int subjectId;
  final String mindmapId;
  final String mindmapName;

  /// 为 true 时打开页面直接跳到 AI 生成 Tab
  final bool openAiTabOnStart;

  const MindmapEditorPage({
    super.key,
    required this.subjectId,
    required this.mindmapId,
    required this.mindmapName,
    this.openAiTabOnStart = false,
  });
...
class _MindmapEditorPageState extends ConsumerState<MindmapEditorPage>
  ...
  void initState() {
    ...
    _tabController = TabController(
      length: 2,
      vsync: this,
      initialIndex: widget.openAiTabOnStart ? 1 : 0,
    );
  }
  ...
  Widget build(BuildContext context) {
    // 确保 nodeTree 已初始化
    ref.watch(subjectMindmapInitProvider(widget.subjectId));

    return Scaffold(
      appBar: AppBar(
        title: Text(widget.mindmapName),
        centerTitle: false,
        bottom: TabBar(
                ...
                children: [
                  ...
                  MindmapToolbar(
                    ...
                    onImportFile: () => handleFileImport(
                        context, ref, widget.subjectId, widget.mindmapId),
        ...
        child: Column(
          ...
          children: [
            ...
            ListTile(
              ...
              onTap: () {
                Navigator.pop(ctx);
                handleExportPng(context, _repaintKey, ref, widget.subjectId,
                    widget.mindmapId);
              },
            ),
...
(async () => {
        ...
        builder: (ctx, setSheet) => Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text('输入主题或粘贴一段文字，AI 自动生成结构化导图',
                ...
                child: Row(
                  children: [
                    ...
                    TextButton(
                      onPressed: () {
                        setState(() {
                          for (final d in docs) { _selectedDocIds.add(d.id); }
                        });
                        setModalState(() {});
                      },
                      child: const Text('全选'),
  ...
  Widget build(BuildContext context) {
    ...
    final chatState = ref.watch(
        chatProvider((widget.subjectId.toString(), 'mindmap')));
              ...
              if (_currentHtml != null) ...[
                Expanded(
                  child: OutlinedButton.icon(
                    onPressed: () =>
                        saveMindMapImage(context, _webViewController),
