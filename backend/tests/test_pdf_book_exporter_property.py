...
# Feature: lecture-book-export, Property 1: èç¹é¡ºåºä¿ç
# Feature: lecture-book-export, Property 3: TOC ä¸æ­£æä¸ä¸å¯¹åº
# Feature: lecture-book-export, Property 4: TOC ç¼©è¿ä¸æ·±åº¦ä¸è?
# Feature: lecture-book-export, Property 9: LaTeX å¬å¼æ¸²æä¸ºåµå¥å¾ç?
"""
...
def _make_png_bytes() -> bytes:
    """Return minimal valid PNG bytes (1Ã1 white pixel)."""
...
def _make_exporter():
    ...
    import book_services.pdf_book_exporter as _mod
...
def nodes_with_depth(draw, min_nodes=1, max_nodes=10):
    """Generate NodeInfo list with explicit depth 1â? and non-empty blocks."""
    n = draw(st.integers(min_value=min_nodes, max_value=max_nodes))
    nodes = []
    for i in range(n):
        node_id = f"node_{i}"
...
def test_toc_titles_match_node_titles_exactly(nodes):
    ...
    set of node titles exactly â?no extras, no omissions.
...
def test_toc_depth_values_match_node_depth(nodes):
    ...
    For any nodes with depth 1â?, each TocEntry must carry the same depth
    ...
    for entry, node in zip(toc_entries, nodes):
        ...
        # depth must be in valid range 1â?
        assert 1 <= entry.depth <= 4, (
            f"TocEntry depth {entry.depth} is outside valid range 1â?"
        )


# ---------------------------------------------------------------------------
# Property 9: LaTeX å¬å¼æ¸²æä¸ºåµå¥å¾ç?
...
def test_latex_blocks_produce_non_empty_pdf(nodes):
    ...
    complete without error and return non-empty bytes â?confirming that image
