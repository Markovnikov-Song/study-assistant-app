...
import 'package:study_assistant_app/routes/app_routes.dart';
...
void main() {
  group('AppRoutes.lecturePage — nodeId URL 编码', () {
    ...
    test('空格被编码为 %20，不是 +', () {
      ...
      // 关键是不能有未编码的空格
      expect(url.contains(' '), isFalse, reason: 'URL 中不应有未编码的空格，实际 URL: $url');
    });

    test('URL 格式正确（路径 + query）', () {
      final url = AppRoutes.lecturePage(9, 72, 'L1_材料力学');
      expect(url, startsWith('/course-space/9/mindmap/72/lecture?node_id='));
    });
    ...
    test('解码后 nodeId 与原始值一致（往返属性）', () {
      ...
      expect(decoded, equals(nodeId), reason: '解码后应与原始 nodeId 完全一致');
    });
